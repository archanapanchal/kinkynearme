...
use App\Yantrana\Components\User\Models\FavouriteModal;
 ...
 * Auth uid
 */
if (!function_exists('fetchTotalUserFavouritedCount')) {
    function fetchTotalUserFavouritedCount($toUserId)
    {
        return FavouriteModal::leftJoin('users', 'favourites.by_users__id', '=', 'users._id')
            ->select(
                __nestedKeyValues([
                    'favourites.*',
                    'users' => [
            ...
            ->where('favourites.to_users__id', $toUserId)
...
if (!function_exists('getStoreSettings')) {
    function getStoreSettings($itemName, $itemKeys = null)
    {
        ...
        // Check if request for admin logo image url
        if ($itemName == 'admin_logo_image_url') {
            $adminLogoName = getStoreSettings('admin_logo_name');
            $adminLogoFilePath = getPathByKey('admin_logo') . '/' . $adminLogoName;
            $adminLogoImageUrl = getMediaUrl($adminLogoFilePath) ?:  url('imgs/' . configItem('admin_logo_name'));
            return $adminLogoImageUrl . '?ver=' . @filemtime($adminLogoFilePath);
        }
        ...
        // Check if request for footer logo image url
        if ($itemName == 'footer_logo_image_url') {
            $footerLogoName = getStoreSettings('footer_logo_name');
            $footerLogoFilePath = getPathByKey('footer_logo') . '/' . $footerLogoName;
            $footerLogoImageUrl = getMediaUrl($footerLogoFilePath) ?:  url('imgs/' . configItem('footer_logo_name'));
            return $footerLogoImageUrl . '?ver=' . @filemtime($footerLogoFilePath);
        }
 ...
 * @return string
 *---------------------------------------------------------------- */
if (!function_exists('fetchUserTopicInterestDetail')) {
    function fetchUserTopicInterestDetail($forumTopicId = null)
    {

        $forumTopic = new ManageForumTopicsRepository();

        $processReaction = $forumTopic->fetchTopicInterestList($forumTopicId);
...
if (!function_exists('getUserPlan')) {
    function getUserPlan($userId) {
        ...
        $planDetail =  User::with(['userSubscription' => function ($query) 
                                    {
                                        $query->with(['userPlan' => function ($query) 
                                            {
                                ...
                                ])->where('status',1)->where('_id',$userId)->first();  
        $userPlan = '';
        if(!empty($planDetail)){
            if(!empty($planDetail->userSubscription->userPlan)){
                 $userPlan = $planDetail->userSubscription->userPlan->title;
            }
...
if (!function_exists('getAdvanceSearchFunction')) {
    function getAdvanceSearchFunction()
    {
        $isOwnProfile = (getUserID() == getUserID()) ? true : false;
        $specificationConfig = App\Yantrana\Components\User\UserEngine::getUserSpecificationConfig();
        ...
        return $userSpecificationData;

    }
}

if (!function_exists('getBlockedUser')) {
    function getBlockedUser($toUserId) {
        $blockUserData = UserBlock::where('to_users__id', $toUserId)
...
if (!function_exists('getMatchUser')) {
    function getMatchUser() {
        //fetch user liked data by to user id
        $likedCollection = UserRepository::fetchUserLikeData(1, false);
        //pluck people like ids
        $peopleLikeUserIds = $likedCollection->pluck('to_users__id')->toArray();
        //get people likes me data
        $userLikedMeData = UserRepository::fetchUserLikeMeData(false)->whereIn('by_users__id', $peopleLikeUserIds);
        //pluck people like me ids
        $mutualLikeIds = $userLikedMeData->pluck('_id')->toArray();
        //get mutual like data
        $matchuser =  $processReaction = UserRepository::fetchMutualLikeUserData($mutualLikeIds,true)->toArray();
        return $matchuser;
    }
}