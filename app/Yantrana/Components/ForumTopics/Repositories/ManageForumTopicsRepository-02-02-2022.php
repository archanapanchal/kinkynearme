...
class ManageForumTopicsRepository extends BaseRepository implements ManageForumTopicsRepositoryBlueprint
{
    ...
    public function fetchTopicDetail($forumTopicUId)
    {
        ...
        $topicDetail =  ForumTopicModel::with('category')
    ...
    public function searchFromHome($searchData)
    {
        ...
        $topic = ForumTopicModel::query();
                    $topic->with('category');
                    $topic->orWhereHas('category',function($query) use($searchData)  {
                        if($searchData){
                            $query->Where('forum_categories.title', 'like', '%' . $searchData . '%');
                        }

                    });
                    $topic->with(['forumInterests' => function ($query) use($searchData)
                        {
                            $query->with('interest');
                            $query->orWhereHas('interest',function($query) use($searchData)  {
                                if($searchData){
                                    $query->Where('kinks.title', 'like', '%' . $searchData . '%');
                                }

                            });
                        }
                    ]);
                    $topic->with('userDetail');
                    if($searchData){
                        $topic->orWhere('forum_topics.title', 'like', '%' . $searchData . '%');
                        $topic->orWhere('forum_topics.description', 'like', '%' . $searchData . '%');
                    }
                    
        $forumTopicData =  $topic->get()->toArray();
        // $forumTopicData =  $topic->toSql();

// echo "<pre>";
//         print_r($forumTopicData);exit();
