...
class ManageAbuseReportEngine extends BaseEngine
{
        ...
        if (!__isEmpty($reportCollection)) {
            ...
            foreach ($reportCollection as $key => $report) {
                $reportListData[] = [
                    '_id'                 => $report['_id'],
                    '_uid'                 => $report['_uid'],
                    'reported_user'     => $report['firstname'].' '.$report['lastname'],
     ...
     * @return object
     *---------------------------------------------------------------- */
    public function processAddAbuseReportReason($inputData)
    {

        $storeReportData = [
            'status' => 1,
            'for_users__id' => $inputData['for_users__id'],
            'by_users__id'  => $inputData['by_users__id'],
            'reason'        =>    $inputData['reason'],
            'moderated_by_users__id' => 1
        ];
