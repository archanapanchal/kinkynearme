...
class MediaEngine extends BaseEngine implements MediaEngineInterface
{
    ...
    public function processForumUpload($input, $folderPath, $requestFor = '', $storeAsPublic = true)
    {

        try {

            $file = $input['forum_images'];
     ...
     * @return array
     *---------------------------------------------------------------- */
    public function processUploadAdminLogo($inputFile, $requestFor)
    {
        $logoFolderPath = getPathByKey('admin_logo');
     ...
     * @return array
     *---------------------------------------------------------------- */
    public function processUploadFooterLogo($inputFile, $requestFor)
    {
       
        $logoFolderPath = getPathByKey('footer_logo');
    ...
    public function processUploadFileOnLocalServer($input, $allowedExtension = '')
    {
        ...
        $path = getPathByKey('user_temp_uploads', ['{_uid}' => $uid]);
    ...
    public function processUploadProfile($inputFile, $requestFor)
    {
        ...
        if ($uploadedFileOnLocalServer['reaction_code'] == 1) {
            ...
            if (isset($inputFile['uid'])) {
                $uid = $inputFile['uid'];
            } else {
                $uid = authUID();
            }
            $fileName = $uploadedFileOnLocalServer['data']['fileName'];
            $profileImageFolderPath = getPathByKey('profile_photo', ['{_uid}' => $uid]);
    ...
    public function processUploadCoverPhoto($inputFile, $requestFor)
    {
        ...
        if ($uploadedFileOnLocalServer['reaction_code'] == 1) {
            ...
            $fileName = $uploadedFileOnLocalServer['data']['fileName'];
            $coverPhotoFolderPath = getPathByKey('cover_photo', ['{_uid}' => $uid]);
     ...
     * @return array
     *---------------------------------------------------------------- */
    public function uploadUserPhotos($inputFile, $requestFor,$userid)
    {
        $uploadedFileOnLocalServer = $this->processUploadFileOnLocalServer($inputFile, $requestFor);
     ...
     * @return array
     *---------------------------------------------------------------- */
    public function processUploadForumFileOnLocalServer($input, $allowedExtension = '')
    {

        print_r($input);exit();
        // if request file not found it will throw error.
        if (!array_has($input, 'forum_images') && __isEmpty($input['forum_images'])) {
            return $this->engineReaction(2, null, __tr('Uploaded file does not exists'));
        }

        $uploadedFile = $input['forum_images'];
     ...
     * @return array
     *---------------------------------------------------------------- */
    public function processUploadForumImage($inputFile, $requestFor)
    {  
        $logoFolderPath = getPathByKey('forum_images',['{_uid}' => authUID()]);

        return $this->processForumUpload($inputFile, $logoFolderPath, $requestFor);
    }
}
