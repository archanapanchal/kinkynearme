...
class UserRepository extends BaseRepository implements UserRepositoryBlueprint
{
    ...
    public function fetchAllUserLikeDislike($userId=null)
    {
        if ($userId == null) {
            $userId = getUserID();
        }
    ...
    public function fetchFavourite($toUserId)
    {
        return FavouriteModal::where('by_users__id', getUserID())
            ->where('to_users__id', $toUserId)
            ->first();
    }
     ...
     * @return eloquent collection object
     *---------------------------------------------------------------- */
    public function fetchWhoViewProfile($toUserId)
    {
        return UserWhoViewProfile::where('to_users__id', $toUserId)
            ->where('by_users__id', getUserID())
            ->first();
    }
     ...
     * @return bool|object
     *---------------------------------------------------------------- */
    public function storeProfileView($input)
    {
        $keyValues = [
            'to_users__id',
            'by_users__id',
        ...
        $userWhoViewProfile = new UserWhoViewProfile;

        // Store user likes and dislikes data
        if ($userWhoViewProfile->assignInputsAndSave($input, $keyValues)) {
            return true;
        }
        return false;
    }
     ...
     * @return eloquent collection object
     *---------------------------------------------------------------- */
    public function fetchAllBlockMeUser($userId = null)
    {   
        if ($userId == null) {
            $userId = getUserID();
        }
        return UserBlock::where('to_users__id', $userId)
            ->get();
    }
     ...
     * @return eloquent collection object
     *---------------------------------------------------------------- */
    public function fetchAllBlockUser($paginate = false, $userId = null)
    {
        if ($userId == null) {
            $userId = getUserID();
        }
        $query =  UserBlock::leftJoin('users', 'user_block_users.to_users__id', '=', 'users._id')
            ...
            ->where('user_block_users.by_users__id', $userId);
     ...
     * @return eloquent collection object
     *---------------------------------------------------------------- */
    public function fetchUserLikeData($like, $paginate = false, $userId = null)
    {
        if ($userId == null) {
            $userId = getUserID();
        }

        $getBlockedUser = UserBlock::where('by_users__id',$userId)->pluck('to_users__id');
        ...
        $query =  LikeDislikeModal::leftJoin('users', 'like_dislikes.to_users__id', '=', 'users._id')
            ...
            ->where('users.status', '=', 1)
            ->where('by_users__id', $userId)
            ->whereNotIn('users._id', $getBlockedUser);;
    ...
    public function prepareWhoViewProfile($paginate = false, $userId = null)
    {
        if ($userId == null) {
            $userId = getUserID();
        }
        ...
        $query =  UserWhoViewProfile::leftJoin('users', 'user_who_view_profiles.by_users__id', '=', 'users._id')
            ->leftJoin('user_authorities', 'users._id', '=', 'user_authorities.users__id')
            ->leftJoin('user_profiles', 'users._id', '=', 'user_profiles.users__id')
            ->leftJoin('countries', 'user_profiles.countries__id', '=', 'countries._id')
            ->select(
                __nestedKeyValues([
                    'user_who_view_profiles.*',
                    // 'like_dislikes.*',
                    'users' => [
                        '_id as userId',
                        '_uid as userUId',
    ...
    public function UserLikeData($like, $paginate = false, $userId = null)
    {
        if ($userId == null) {
            $userId = getUserID();
        }
    ...
    public function UserLikeDataPerDay($like, $paginate = false, $userId = null)
    {
        if ($userId == null) {
            $userId = getUserID();
        }
        ...
        $query =  LikeDislikeModal::leftJoin('users', 'like_dislikes.to_users__id', '=', 'users._id')
            ...
            ->where('like_dislikes.like', $like)
            ->whereDate('like_dislikes.created_at', Carbon::today())
            ->where('users.status', '=', 1)
            ->where('by_users__id', $userId)
            ->whereNotIn('users._id', $getBlockedUser);;
        if ($paginate) {
    ...
    public function UserLike($like, $paginate = false, $userId = null)
    {
        if ($userId == null) {
            $userId = getUserID();
        }
    ...
    public function UserLikeYou($like, $paginate = false, $userId = null)
    {
        if ($userId == null) {
            $userId = getUserID();
        }
     ...
     * @return eloquent collection object
     *---------------------------------------------------------------- */
    public function fetchUserLikeMeData($paginate = false, $userId = null)
    {
        if ($userId == null) {
            $userId = getUserID();
        }
     ...
     * @return eloquent collection object
     *---------------------------------------------------------------- */
    public function fetchMutualLikeUserData($mutualLikeIds, $paginate = true, $userId = null)
    {
        if ($userId == null) {
            $userId = getUserID();
        }
    ...
    /**
     * Store user favourite data.
     *
     * @param array $storeData
     *
     *-----------------------------------------------------------------------*/
    public function storeFavourite($storeData)
    {
        $keyValues = [
            'status',
            'to_users__id',
        ...
        // Get Instance of user likes dislikes model
        $favouriteModal = new FavouriteModal;

        // Store user likes and dislikes data
        if ($favouriteModal->assignInputsAndSave($storeData, $keyValues)) {
            return true;
        }
        return false;
    }
     ...
     * @return bool
     *---------------------------------------------------------------- */
    public function updateFavourite($favouriteData, $updateData)
    {
        // Check if information updated
        if ($favourite->modelUpdate($updateData)) {
            return true;
        }
    ...
    /**
     * delete favourite user data.
     *
     * @param array $storeData
     *
     *-----------------------------------------------------------------------*/
    public function deleteFavourite($favouriteData)
    {
        // Check if like dislike user data deleted
        if ($favouriteData->delete()) {
            return  true;
        }
    ...
    /**
     * Fetch the record of User favourited
     *
     *
     * @return eloquent collection object
     *---------------------------------------------------------------- */
    public function fetchUserFavouriteData($favourite, $paginate = false, $userId=null)
    {
        if ($userId == null) {
            $userId = getUserID();
        }
        ...
        $query =  FavouriteModal::leftJoin('users', 'favourites.to_users__id', '=', 'users._id')
            ->leftJoin('user_authorities', 'users._id', '=', 'user_authorities.users__id')
            ->leftJoin('user_profiles', 'users._id', '=', 'user_profiles.users__id')
            ->leftJoin('countries', 'user_profiles.countries__id', '=', 'countries._id')
            ->select(
            ...
            ->where('favourites.favourite', $favourite)
            ->where('users.status', '=', 1)
            ->where('by_users__id', $userId)
            ->whereNotIn('users._id', $getBlockedUser);
        if ($paginate) {
    ...
    /**
     * Fetch the record of User favourite
     *
     *
     * @return eloquent collection object
     *---------------------------------------------------------------- */
    public function fetchUserFavouriteMeData($paginate = false, $userId = null)
    {
        if ($userId == null) {
            $userId = getUserID();
        }
        ...
        $query =  FavouriteModal::leftJoin('users', 'favourites.by_users__id', '=', 'users._id')
            ->leftJoin('user_authorities', 'users._id', '=', 'user_authorities.users__id')
            ->leftJoin('user_profiles', 'users._id', '=', 'user_profiles.users__id')
            ->leftJoin('countries', 'user_profiles.countries__id', '=', 'countries._id')
            ->select(
    ...
    public function searchRequestFromHome($searchData,$paginate = false,$checkUserStatusForAdmin = false)
    {
       $query = UserModel::leftJoin('user_authorities', 'users._id', '=', 'user_authorities.users__id')
            ->select(
                __nestedKeyValues([
                    'users.*',
                    'user_authorities' => [
