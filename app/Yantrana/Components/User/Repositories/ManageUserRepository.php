...
class ManageUserRepository extends BaseRepository
{
    ...
    public function fetchUsersDataTableSource($status)
    {
        $dataTableConfig = [
            'searchable' => [
                ...
                'user_profiles.city',
                'countries.name',
                'location' => DB::raw("CONCAT(COALESCE(user_profiles.city, ''), ' ', COALESCE(countries.name, ''))"),
            ]
        ];

        $query = User::leftJoin('user_profiles', 'users._id', '=', 'user_profiles.users__id')
            ...
            ->select(
                __nestedKeyValues([
                    ...
                    DB::raw("CONCAT(COALESCE(user_profiles.city, ''), ' ', COALESCE(countries.name, '')) AS location"),
                ])
            )
    ...
    public function fetchUsersExportSource($status)
    {
        $query = User::leftJoin('user_profiles', 'users._id', '=', 'user_profiles.users__id')
            ->leftJoin('user_authorities', 'users._id', '=', 'user_authorities.users__id')
            ->leftJoin('countries', 'user_profiles.countries__id', '=', 'countries._id')
