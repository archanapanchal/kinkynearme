...
class UserController extends BaseController
{
     ...
     *-----------------------------------------------------------------------*/
    public function __construct(ManageUserEngine $manageUserEngine,UserEngine $userEngine, ManagePlansEngine $managePlanEngine,UserRepository $userRepository, ManageForumTopicsEngine $manageForumTopicsEngine, ManageAbuseReportEngine $manageAbuseReportEngine)
    {
    ...
    public function settinig_profile_insert_update(CommonUnsecuredPostRequest $request){
        ...
        // echo "<pre>";
        // print_r($Update_profile_setting);exit();
   ...
   public function updateUserProfile(Request $request){
    ...
    public function getLikeYouView(CommonUnsecuredPostRequest $request)
    {
    ...
    public function messages(CommonUnsecuredPostRequest $request)
    {
       /* $user_subscription_detail = UserSubscription::where('users__id', getUserID())->where('status', 1)->get()->toArray();
        ...
        $processReaction['data']['message_check_data'] =  $processReaction['data']['usersData'];
        /*search*/
        $processReaction['data']['search_data'] = $search_data;
        $processReaction['data']['search_5_data'] = $search_5_data;
        $processReaction['data']['advance_search'] = 0;
    ...
    public function whoViewProfile(CommonUnsecuredPostRequest $request)
    {


        if((getUserPlan(Auth::user()->_id) == config('constant.trial')) || (getUserPlan(Auth::user()->_id) == config('constant.silver'))){
            abort(404);
        }
        ...
        $page = request()->input('page');
        //get favourite people data by parameter like '1'
        $processReaction = $this->userEngine->prepareWhoViewProfileData();
        ...
        $processReaction['data']['active_tab'] = 9;
        $userMatchData = getMatchUser();
        $processReaction['data']['userMatchData'] = $userMatchData;
        // echo "<pre>";
        // print_r($processReaction['data']);exit();
    ...
    public function frind_profile_data(Request $request){
            ...
            // print_r($usersData['userId']);exit();

            $user_who_view_profile = $this->userEngine->addUserWhoViewProfile($processReaction['data']['userData']['userId']);

            // print_r($user_who_view_profile);exit();
            ...
            if (isset($processReaction['data'])) {
               ...
               if (isset($processReaction['data']['userOnlineStatus'])) {
                ...
                if (isset($processReaction['data']['userSpecifications']['eye_color'])) {
                ...
                $response_layout .="</div>
                                ...
                                <a class='user_abuse_report' forUserId='".$userId."' byUserId='".getUserID()."'><div class='address-social-icon abuse-report-link-section d-flex-class'>
                                    <div class='about-report-link'>
                                            Report Abuse
                                    </div>                                    
                                </div></a>
                                <a class='block_user_section' forUserId='".$userId."' byUserId='".getUserID()."' status='1'><div class='address-social-icon block-user-section d-flex-class'>
                                    <div class='about-block-user-link'>
                                            Block User
                                    </div>                                    
                                </div></a>";
            ...
            $response_layout .="</div>
                        ...
                        $response_layout .= "<div class='physical-appearance'>
                        ...
                        $response_layout .= "<div class='physical-appearance life-style'>
                                            <div class='open-desc d-flex-class'>
                                                <span>Lifestyle</span>
                                            </div>
                                            <div class='open-desc'>
        ...
        $response_layout .="<div class='my-profile-images'>
                                ...
                                <div class='row'>";
                                    foreach($photosData as $key => $photo){
                                        $response_layout .="<div class='col-md-3'><img username='".$username."' class='lw-user-photo lw-photoswipe-gallery-img lw-lazy-img user-detail-gallery-image' data-img-index=".$key." src=". imageOrNoImageAvailable($photo['image_url'])."></div>";
                                    }
                               $response_layout .="</div>
                            </div>";
                            }else{
                                $response_layout .="<p>".__tr('Oops... No images found...')."</p>";
                            }
    ...
    public function userDetailGalleryImage(Request $request)
    {
        $username = $request->input('username');
        $processReaction = $this->userEngine->prepareUserProfile($username);
        ...
        if (isset($processReaction['data'])) {
            ...
            if(!__isEmpty($photosData)){
                $response_layout = '';
                $response_layout.='<div id="carousel-example-1z" class="carousel slide carousel-fade" data-ride="carousel">
                        <ol class="carousel-indicators">';
                foreach ($photosData as $key => $photo) {
                    $response_layout.='<li data-target="#carousel-example-1z" data-slide-to="'.$key.'"></li>';
                }
                $response_layout.='</ol>
                ...
                foreach ($photosData as $key => $photo) {
                    ...
                    $response_layout.= '<div class="carousel-item">
                                <img class="d-block w-100" src="'. imageOrNoImageAvailable($photo['image_url']).'" alt="'.$key.' slide" />
                            </div>';
                }
                $response_layout.= '</div>
    ...
    public function add_user_abuse_report_reason(Request $request)
    {
        // echo "<pre>";
        // print_r($request->all());
        $forUserId = $request['forUserId'];
        $byUserId = $request['byUserId'];
        $responseLayout = '<form class="user abuse-report-modal lw-ajax-form lw-form" id="abuse-report-form">
                    <input type="hidden" class="for_users__id" name="for_users__id" value="'.$forUserId.'">
                    <input type="hidden" class="by_users__id" name="by_users__id" value="'.$byUserId.'">
                    <div class="form-group abuse-report-reason">
                        <label for="lwReason">Report</label>
                        <textarea rows="2" cols="30" class="form-control summernote" id="lwReason" name="reason" required></textarea>
                    </div>
                    <button class="submitButton btn btn-primary btn-user btn-block-on-mobile">Report</button>
                </form>';
        return $responseLayout;
    }

    public function process_add_user_abuse_report_reason(AbuseReportRequest $request)
    {
        $processReaction = $this->manageAbuseReportEngine
            ->processAddAbuseReportReason($request->all());
    ...
    public function add_block_user_view(Request $request)
    {
        // echo "<pre>";
        // print_r($request->all());
        $forUserId = $request['forUserId'];
        $byUserId = $request['byUserId'];
        $responseLayout = '<form class="user block-user-modal lw-ajax-form lw-form" method="post" action="#">
                            <input type="hidden" class="for_users__id" name="for_users__id" value="'.$forUserId.'">
                            <input type="hidden" class="by_users__id" name="by_users__id" value="'.$byUserId.'">
                            <input type="hidden" class="status" name="status" value="1">
                                            '. __tr("Are you sure you want to block this user?") .'
                                            <hr />
                                            <button type="submit" class="blockUserButton btn btn-primary btn-user btn-block-on-mobile">'.__tr('Block User') .'</button>
                                        </form>';
        return $responseLayout;
    }

     public function process_add_block_user(Request $request)
    {
        $forUserId['block_user_id'] = $request['for_users__id'];
        $processReaction = $this->userEngine->processBlockUser($forUserId);
    ...
    public function advance_search(Request $request){
            ...
            foreach($processReaction['data']['usersData'] as $keys => $value){



                $specifications = UserSpecificationModel::where('users__id',$value['_id'])->get()->toArray();
                foreach ($specifications as $key => $values) {
                  if($values['specification_key'] == 'our_sexual_orientation'){
                    $our_sexual_orientation = $values['specification_value'];
                  }
                  if($values['specification_key'] == 'kinks'){
                    $kinks = $values['specification_value'];
                  }
                    //$processReaction['data'][$keys]['our_sexual_orientation'] = $our_sexual_orientation;
                }
            }
            
            $processReaction['data']['search_data'] = $processReaction['data']['usersData'];
            $processReaction['data']['totalCount'] = $countUser;            
            $processReaction['data']['active_tab'] = 0;
            ...
            $responce_user_data['data'][$key]['kinks'] = $kinks;
