...
class UserForumController extends BaseController
{
     ...
     *-----------------------------------------------------------------------*/
    public function __construct(ManageForumTopicsEngine $manageForumTopicsEngine,ManageForumTopicsRepository $manageForumTopicsRepository, ManageForumCategoriesEngine $manageForumCategoriesEngine,MediaEngine $mediaEngine,UserRepository $userRepository,BaseMailer  $baseMailer)
    {
    ...
    public function deleteTopic($forumTopicUId)
    {
        ...
        $last_seg = end($link_array);

        if($last_seg == 'topic-detail'){
            $link = 'user.view.discussion-forum';
        }else{
            $link = $url;
        }
    ...
    public function replyToTopic(ManageForumTopicReplyRequest $request)
    {
        ...
        //check reaction code equal to 1
        if ($processReaction['reaction_code'] === 1) {
            $formtopics = DB::table('forum_topics')->where('_id',$request->topic_id)->where('users__id','!=','1')->first();

            $user = $this->userRepository->fetch($formtopics->users__id);
            $loggedInUserName = Auth::user()->username;
            /*email*/
            $emailData = [
                'FromUserName' => $loggedInUserName,
                'ToUserName'   => $user->username,
                'PageURL'      => route('user.view.discussion-forum'),
                'topic'        => $request->topic_title,
                
            ];

            $matchkey = 'someone_comments_topic';
            $userstatus = DB::table('user_settings')->where('users__id',$user->_id)->where('key_name','LIKE','%'.$matchkey.'%')->where('value','1')->get()->toArray();

            if(!empty($userstatus)){
                $this->baseMailer->notifyToUser('KinkyNearMe - Forum Comment', 'forumtopic', $emailData,$user->email);
            }
            return $this->responseAction(
                $this->processResponse($processReaction, [], [], true),
                $this->redirectTo(url()->previous())
    ...
    public function viewAllForumResultSearch(Request $request)
    {
        ...
        if($search != ""){
            ...
            if(!empty($search_data_forum)){
                $forum_count = count($search_data_forum);
            }
        }
        // echo "<pre>";
        // print_r($search_data_forum);exit();

        $processReaction['data']['active_tab'] = 0;
        $processReaction['data']['search_forum_data'] = $search_data_forum;
        $processReaction['data']['totalCount'] = $forum_count;
        ...
        return $this->loadProfileView('filter.view-all-forum-results', $processReaction['data']);
