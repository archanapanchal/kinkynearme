...
implements UserSettingRepositoryInterface
{
    ...
    public function fetchUserSettingsProfile($userId)
    {
        return UserSettingModel::where('users__id', $userId)->get()->toArray();
    }
    ...
    public function updateUserDataFront($user_data)
    {

            $user_data_collection = User::where('_id', getUserID())->get()->toArray();

            foreach ($user_data_collection as $key => $udata_coll) {
                //$udata_coll->first_name
                $udata_coll_key_array = array_keys($user_data);
                foreach ($udata_coll as $key => $udata) {
                    
                    if(in_array($key, $udata_coll_key_array)){

                        $update_user_data = User::where('_id', getUserID())->update(array($key => $user_data[$key]));
    ...
    public function updateUserSpecificationFront($userSpecificationcollection)
    {
        $UserSpecificationModelcollection =  UserSpecificationModel::where('users__id', getUserID())->pluck('specification_key','_id')->toArray();

            
        $Specificationcollectionkey = array_keys($userSpecificationcollection);
        foreach ($Specificationcollectionkey as $key => $Modelcollection) {
            if (in_array($Modelcollection,$UserSpecificationModelcollection)) {
                $affectedRows = UserSpecificationModel::where('users__id',getUserID())->where('specification_key',$Modelcollection)->update(array('specification_value' => $userSpecificationcollection[$Modelcollection]));
                ...
                $userSpecCreate = [
                    ...
                    'type' =>1,
                    'status' =>1,
                    'specification_key'     => $Modelcollection,
                    'specification_value'   => $userSpecificationcollection[$Modelcollection],
                    'users__id' => getUserID(),
                ];
                UserSpecificationModel::insert($userSpecCreate);
        ...
        // $UserSpecificationModelcollection =  UserSpecificationModel::where('users__id', getUserID())->get()->toArray();


        // $Specificationcollectionkey = array_keys($userSpecificationcollection);
        // foreach ($UserSpecificationModelcollection as $key => $Modelcollection) {
   
                    
        //     if (in_array($Modelcollection['specification_key'],$Specificationcollectionkey)) {
                    
        //         $affectedRows = UserSpecificationModel::where('users__id',getUserID())->where('specification_key',$Modelcollection['specification_key'])->update(array('specification_value' => $userSpecificationcollection[$Modelcollection['specification_key']]));
        //     }
        // }
    ...
    public function updateUserProfileFront($updateProfileDetail)
    {
            $UserProfiledata =  UserProfile::where('users__id',getUserID())->get()->toArray();
            foreach ($UserProfiledata as $key => $Profiledata) {
                    foreach ($Profiledata as $key => $Profile) {
                        $updateProfileDetailkey = array_keys($updateProfileDetail);
                           // echo "<pre>";print_r($key);
                        if (in_array($key,$updateProfileDetailkey)) {
                            $affectedRows = UserProfile::where('users__id',getUserID())->update(array($key => $updateProfileDetail[$key]));
                        }
                    }
                   // exit();
            }
    ...
    public function fetchUserVideos($userId)
    {
        return UserVideosModel::where('users_id', $userId)->get();
    }
     ...
     * @return eloquent collection object
     *---------------------------------------------------------------- */
    public function fetchUserPhotosById($photoUid,$Uid)
    {
    ...
    public function searchStaticCities($searchQuery)
    {
        $searchable = [
            'cities.name',
            // 'states.name',
            // 'cities_full_name' => DB::raw("CONCAT(cities.name, states.name, countries.name)")
        ];

        return CityModel::select([
            DB::raw("CONCAT(cities.name, ', ', states.name, ', ', countries.name) AS cities_full_name"),
            "cities.id"
        ])
            ->join('states', 'cities.state_id', '=', 'states.id')
            ->join('countries', 'cities.country_code', '=', 'countries.iso_code')
            ->shodh($searchQuery, $searchable)
            ->take(100)
            ->get()
            ->toArray();
    }
    ...
    /**
     * Fetch all user specifications
     *
     * @return eloquent collection object
     *---------------------------------------------------------------- */

    public function fetchAllUsersSpecification()
    {
        return UserSpecificationModel::where('status', 1)->get();
    }
}
