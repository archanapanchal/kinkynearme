<div class="lw-messenger-header row">
    <div class="col-md-11">
        ...
        <div class="float-right pt-3">
            <?php /* Disabled Temporarily
            @if(getStoreSettings('allow_pusher') and getStoreSettings('allow_agora'))
            @if(getFeatureSettings('audio_call_via_messenger'))
            <a class="btn btn-primary rounded-circle lw-audio-video-btns" id="lwAudioCallDisableBtn"><i class="fas fa-phone-alt" title="<?= __tr('Audio Call') ?>"></i></a>
            <!-- audio call button -->
            <a class="btn btn-primary rounded-circle lw-audio-video-btns" href type="button" id="lwAudioCallBtn" data-user-uid="<?= $userData['user_uid'] ?>" data-call-type="audio" data-confirm="#lwCallErrorContainer" title="<?= __tr('Audio Call') ?>"><i class="fas fa-phone-alt"></i></a>
            <!-- /audio call button -->
            @endif

            @if(getFeatureSettings('video_call_via_messenger'))
            <a class="btn btn-primary rounded-circle lw-audio-video-btns" id="lwVideoCallDisableBtn" readonly><i class="fas fa-video" title="<?= __tr('Video Call') ?>"></i> </a>
            <!-- video call button -->
            <a class="btn btn-primary rounded-circle lw-audio-video-btns" href type="button" id="lwVideoCallBtn" data-user-uid="<?= $userData['user_uid'] ?>" data-call-type="video" data-confirm="#lwCallErrorContainer" title="<?= __tr('Video Call') ?>"><i class="fas fa-video"></i> </a>
            <!-- /video call button -->
            @endif
            @endif
            */ ?>
    ...
    <div class="col-md-1 text-right lw-messenger-user-menu">
        <div class="dropdown">
        ...
        <form class="lw-ajax-form lw-form" method="post" action="<?= route('user.write.send_message', ['userId' => $userData['user_id']]) ?>" id="lwSendMessageForm" style="display: none;">
            <div class="input-group">
                ...
                <div class="input-group-append" id="button-addon4">
                    ...
                    <!-- Sticker Button -->
                    <a class="btn btn-outline-secondary lw-open-stickers-action lw-ajax-link-action d-none" href="<?= route('user.read.get_stickers') ?>" data-callback="__Messenger.fetchStickers"><i class="fas fa-sticky-note"></i></a>
                    <!-- /Sticker Button -->
