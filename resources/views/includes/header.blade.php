...
<?php
	$userSpecificationData = getAdvanceSearchFunction();
	...
	<div class="wrapper">
      <div class="advance-search advance-search-v2" style="display: none;">
         ...
         @guest
			...
			@else
			<form method="get" action="{{ route('user.advance-search', ['userUid' => getUserID()]) }}">
			@endguest
            ...
            <div class="cty-st-ctry-age-rang row">
               <div class="cty-sts-ctry col-md-6">
                  ...
                  <div class="cty-sts-ctry-btn">
                     <div class="city"> 
                     	<!-- <a href="#">Los Angeles<i class="fa fa-times"></i></a> <a href="#">San Fransisco<i class="fa fa-times"></i></a>  -->
                     	<input type="hidden" name="city_id" id="cityId">
	                     <input type="text" id="selectLocationCityAdvanceSearch" class="form-control"  value="">
                     </div>
                  </div>
               </div>
			         ...
			         <p>Age Range</p>
							<div slider id="slider-distance">
								...
								<div sign style="left:30%;">
									<span id="value">23</span>
								</div>
								<div sign style="left:60%;">
									<span id="value">40</span>
								</div>
								</div>
							<input type="range" id="age_min_range" name="age_min_range" tabindex="0" value="30" max="100" min="18" step="1" oninput="
							...
							<input type="range" id="age_max_range" name="age_max_range" tabindex="0" value="60" max="100" min="0" step="1" oninput="
               ...
               <div class="physical-app col-md-6">
                  ...
                  <div class="select-bar row">
                     <div class="form-field col-md-6">
                        <label><img src="<?php echo url('/dist/images/app-2.png'); ?>" />Hair Color</label>
                     ...
                     <div class="form-field col-md-6">
                        <label><img src="<?php echo url('/dist/images/app-5.png'); ?>" />Eye Color</label>
                        <div class="select-input">
                           @foreach(collect($userSpecificationData['step-2']['items'])->chunk(2) as $specification) 
		                        @foreach($specification as $itemKey => $item)
                           ...
                           @foreach(collect($userSpecificationData['step-4']['items'])->chunk(2) as $specification) 
		                        @foreach($specification as $itemKey => $item)
                                    ...
                                    <select name="no_of_children" id="select_gender">
                                        ...
                                        @foreach($item['options'] as $genderKey => $options)
                                        <option value="<?= $genderKey ?>" <?= $item['selected_options'] == $genderKey ? '' : '' ?>><?= $options ?></option>
                                        @endforeach
                                    </select>
                                	@endif   
	                           @endforeach
                     ...
                     <div class="form-field col-md-6">
			                                    ...
			                                    <div class="form-field col-md-6">
			                                       ...
			                                       <div class="select-input">
			                                       	@foreach(collect($userSpecificationData['step-1']['items'])->chunk(2) as $specification) 
							                           @foreach($specification as $itemKey => $item)
							                                   @if($item['name'] == 'Sexual preference')
                        ...
                        <div class="select-input">
                           @foreach(collect($userSpecificationData['step-2']['items'])->chunk(2) as $specification) 
		                        @foreach($specification as $itemKey => $item)
                                    ...
                                    <select name="married" id="body_type">
                                        <option value="">Select</option>
                                        @foreach($item['options'] as $genderKey => $options)
                                        <option value="<?= $genderKey ?>" <?= $item['selected_options'] == $genderKey ? 'selected' : '' ?>><?= $options ?></option>
                                        @endforeach
            ...
            <div class="show-mamber">
               ...
               <div class="check-member-box">
                  <div class="form-group check-box">
                     <input type="checkbox" id="html" value="1" name="currently_online"/>
                     <label for="html">Who are currently Online</label>
                  </div>
                  <div class="form-group check-box">
                     <input type="checkbox" id="css" value="1" name="have_photos"/>
                     <label for="css">Who have Photos</label>
                  </div>
                  <div class="form-group check-box">
                     <input type="checkbox" id="javascript" value="1" name="has_videos"/>
      ...
      $currentURL = URL::current();
			...
			if (strpos($currentURL, $key) == false) { 
	       	if (!empty($search_data)) { 
	       				...
	       				//echo "<pre>";print_r($advance_search);exit(); 
	       		
	       		if ($advance_search != 1) { ?>

	       			
	       				
	       			<div class="advance-search commmon-search-tab" id="commmon-search-tab" style="display: block;">
		            <div class="row">
		            	<a type="button" class="close-search-popup">X</a>
		                  ...
		                  <div class="advance-search-left">
		                     ...
		                     <?php } else {
		                     	if(!empty($search_5_data)){
		                        foreach ($search_5_data as $key => $search) { 
		                           ?>
		                           <div class="block-profile-left-side d-flex-class">
		                                 <div class="img-left-side"><img src="<?php echo $search['profilePicture']; ?>"></div>
		                                 <div class="content-left-side">
		                                    <h5 id="display_user_detail"  class="display_user_detail" value="<?= $search['username'];?>" ><?php echo $search['username']; ?></h5>
		                                 </div>
		                              </div>
		                     <?php } }
		                    ...
		                    //	echo "<pre>";print_r($search_data['search_data']);exit();
		                    ...
		                    <form action="<?= route('user.read.view-search-results') ?>" method="get">
				               	<div class="input-group">
			                    	<div class="form-outline">
			                        <input type="hidden" name="search" id="form1" value="<?= $search_string ?>" />
			                    	</div>
				               	</div>
		                 		<button type="submit" class="btn btn-outline-secondary"> view-all</button>
					        </form>
		                    @endif
		                  </div>
		               </div>

		               <?php if (!empty($search_data_forum)) { ?>
		               <div class="col-md-8">
		                  <div class="advance-search-right">
		                     <h5>Forum Topics</h5>
		                     <div class="advance-search-right-inner">
		                        <div class="advance-search-content">
		                           <?php if(!empty($search_5_data_forum)){
			                        foreach ($search_5_data_forum as $key => $search_forum) { ?>
		                           <a href="{{url('/user/').'/'.$search_forum['_uid'].'/topic-detail'}}"><p>{{$search_forum['title']}}</p></a>
		                           <br>
		                        	<?php } }  ?>
		                        </div>
		                     </div>
		                     @if(count($search_data) > 5)
		                    	<form action="<?= route('user.read.forum.view-search-forum-results') ?>" method="get">
				               	<div class="input-group">
				                    	<div class="form-outline">
				                        <input type="hidden" name="search" id="form2" value="<?= $search_string ?>" />
				                    	</div>
				               	</div>
		                 			<button type="submit" class="btn btn-outline-secondary"> view-all</button>
					        		</form>
		                        	...
		                        	<div class="select-input">
		                            	<div class="dropdown">
	                            			...
	                            			<div class="dropdown-menu">
		                                		<a class="dropdown-item" href="<?= route('landing_page') ?>"><?= __tr('My Account') ?></a>
