...
<div class="modal fade bd-example-modal-lg abuse_report_modal" tabindex="-1" role="dialog" aria-labelledby="myLargeModalLabel" aria-hidden="true">
      <div class="modal-dialog modal-lg">
        <div class="modal-content">
            <div class="modal-header">
            ...
            <div class="tab-pane active" id="tabs-6" role="tabpanel">
                <div class="my-abuse-report-account">
                   <div id="my-abuse-report-profile" class="tabcontentreport">
    ...
    <div class="modal fade bd-example-modal-lg block_user_modal" tabindex="-1" role="dialog" aria-labelledby="myLargeModalLabel" aria-hidden="true">
      <div class="modal-dialog modal-lg">
        <div class="modal-content">
            <div class="modal-header">
                <h5 class="modal-title"><?= __tr('Block User?') ?></h5>
                <button type="button" class="close" data-dismiss="modal" aria-label="Close">
                  <span aria-hidden="true">×</span>
                </button>
              </div>
            <div class="tab-pane active" id="tabs-6" role="tabpanel">
                <div class="block-user-account">
                   <div id="block-user-profile" class="tabcontentblockuser">
    ...
    <div class="modal fade bd-example-modal-lg user_detail_gallery_modal" tabindex="-1" role="dialog" aria-labelledby="myGalleryModalLabel" aria-hidden="true">
      <div class="modal-dialog modal-lg">
        <div class="modal-content">
            <div class="modal-header">
                <h5 class="modal-title"><?= __tr('User Gallery') ?></h5>
                <button type="button" class="close" data-dismiss="modal" aria-label="Close">
                  <span aria-hidden="true">×</span>
                </button>
              </div>
            <div class="tab-pane active" id="tabs-6" role="tabpanel">
                <div class="user-detail-gallery">
                   <div id="user-gallery" class="tabcontentusergallery">
...
<script>
    ...
    $(document).on("click",".submitButton", function(event){
        ...
        var for_users__id = $('.for_users__id').val();
        var by_users__id = $('.by_users__id').val();
        var lwReason = $('#lwReason').val();
        $.ajax({
            url: "{{route('user.process_add_user_abuse_report_reason')}}",
           method: 'POST',
           data: {
                for_users__id: for_users__id,
                by_users__id: by_users__id,
                reason: lwReason
            },
           success: function(response){
                $('.abuse_report_modal').modal('hide');
           },
    ...
    $(document).on("click",".blockUserButton", function(event){
        ...
        $.ajax({
            url: "{{route('user.process_add_block_user')}}",
           method: 'POST',
           data: {
                for_users__id: for_users__id,
                by_users__id: by_users__id,
           ...
           success: function(response){
                ...
                $('#user-'+for_users__id).css('display','none');

                $('.block_user_section').css('display','none');
           },
           error: function(response){
              console.log("error : " + JSON.stringify(response) );
           }
    ...
    $('#age_min_range').change(function(){

    var range = $(this).val();

    $(this).val(range);
  });
  $('#age_max_range').change(function(){
    ...
    $(document).on('click','.user-detail-gallery-image', function(e){
        
        var username = $(this).attr('username');

        var requestUrl = '<?= route('user.user_detail_gallery_image') ?>';
        ...
        $.ajax({
            ...
            success: function(response){ 
              $('.tabcontentusergallery').html(response);
                $('.user_detail_gallery_modal').modal('show'); 
    ...
    $(document).on('click','.user_abuse_report', function(e){

        var forUserId = $(this).attr('forUserId');

        var byUserId = $(this).attr('byUserId');
        
        var requestUrl = '<?= route('user.add_user_abuse_report_reason') ?>';
        ...
        $.ajax({
            ...
            success: function(response){ 
              $('.tabcontentreport').html(response);
                $('.abuse_report_modal').modal('show'); 
            }
          });
    });

    $(document).on('click','.block_user_section', function(e){
        ...
        var requestUrl = '<?= route('user.add_block_user_view') ?>';
        ...
        $.ajax({
            ...
            success: function(response){ 
              $('.tabcontentblockuser').html(response);
...
<script>
        $(document).ready(function(){
            ...
            $(document).on('click','#forumtopictitle', function(){
                var topic_id = $(this).attr('topic_id');
                var requestUrl = '<?= route('user.write.count-user-forum-topic') ?>';
                $.ajax({
                    url: requestUrl,
                    type: 'post',
                    data: {
