...
<div class="card mb-3">
    <div class="card-body">
        @if($photosCount <= 10) <input type="file" class="lw-file-uploader" data-instant-upload="true" data-action="<?= route('user.upload_photos', ['userid' => $userDetails->_id]) ?>" data-default-image-url="" data-allowed-media='<?= getMediaRestriction('photos') ?>' multiple data-callback="afterFileUpload" data-remove-all-media="true">
            @endif
...
<script type="text/_template" id="lwPhotosContainer">
    ...
    <% _.forEach(__tData.userPhotos, function(item, index) { %>
       <div class="lw-photo-thumbnail">
        <!-- delete photo button -->
        <a class="btn btn-danger btn-sm lw-remove-photo-btn lw-ajax-link-action" href="<%- item.removePhotoUrl %>"data-callback="onDeletePhotoCallback" data-method="post"><i class="far fa-trash-alt"></i></a>
        <!-- /delete photo button -->
        <!-- user photos -->
