...
<div class="col-lg-3 mb-5 load-matches" id="user-<?= $user['id'] ?>" style="display: block;">
    <div class="profile-people">
        <a href="javascript:void(0)">
            <img data-src="<?= imageOrNoImageAvailable($user['profileImage']) ?>" class="lw-user-thumbnail lw-lazy-img" />
        ...
        <div class="profile-desc">
            <div class="title-icon d-flex">
                ...
                <?php } ?>
                <h4 id="display_user_detail"  class="display_user_detail" value="<?= $user['username']?>" ><?= $user['fullName'] ?></h4>
            ...
            <div class="profile-icon d-flex">
                <div class="profile-icon-left-side d-flex">
                    <div class="main-heart">
                        <a href data-action="<?= route('user.write.like_dislike', ['toUserUid' => $user['_uid'], 'like' => 1]) ?>" data-method="post" data-callback="onLikeCallback" title="Like" class="icon lw-ajax-link-action lw-like-action-btn" id="lwLikeBtn">

                            <?php 
                            if (in_array($user['id'],$current_user_mutual_like_ids)) { ?>

                            <span class="heart-icon" style="display:none;">
                           ...
                           <?php 
                            ...
                            <span class="hover-show" style="display:block">
                                <svg xmlns="http://www.w3.org/2000/svg" width="22.002" height="19.503" viewBox="0 0 22.002 19.503">
                                    <g id="Component_2_19" data-name="Component 2 – 19" transform="translate(1.001 1.003)">
                                        <path id="Path_280" data-name="Path 280" d="M18.059-15.055a5.342,5.342,0,0,0-7.289.531L10-13.73l-.77-.793a5.341,5.341,0,0,0-7.289-.531,5.609,5.609,0,0,0-.387,8.121L9.113.871a1.225,1.225,0,0,0,1.77,0l7.559-7.8A5.606,5.606,0,0,0,18.059-15.055Z" transform="translate(0 16.251)" fill="#f06a6a" stroke="rgba(240,106,106,0)" stroke-width="2"/>
                                    </g>
                                </svg>
                            </span>
                        <?php } ?>
                    ...
                    <div class="main-star">
                        <a href data-action="<?= route('user.write.favourite', ['toUserUid' => $user['_uid'], 'favourite' => 1]) ?>" data-method="post" data-callback="onFavouriteCallback" title="Favorite" class="icon lw-ajax-link-action lw-favourite-action-btn" id="lwFavouriteBtn">

                            <?php 
                            if (in_array($user['id'],$current_user_favourite_ids)) { ?>
                            <span class="star-icon" style="display:none">
