...
<div class="col-lg-3 mb-5 load-matches" id="user-<?= $user['_id'] ?>">
    <div class="profile-people">
        <a href="<?= route('user.profile_view', ['username' => $user['username']]) ?>">
            <img data-src="<?= imageOrNoImageAvailable($user['profilePicture']) ?>" class="lw-user-thumbnail lw-lazy-img" />
        </a>
                ...
                <div class="profile-icon-left-side d-flex">
                    <div class="main-heart">
                        <a href data-action="<?= route('user.write.like_dislike', ['toUserUid' => $user['_uid'], 'like' => 1]) ?>" data-method="post" data-callback="onLikeCallback" title="Like" class="icon lw-ajax-link-action lw-like-action-btn" id="lwLikeBtn">
                            ...
                            <?php 
                            if (in_array($user['_id'],$current_user_mutual_like_ids)) { ?>
                    ...
                    <div class="main-star">
                        <a href data-action="<?= route('user.write.favourite', ['toUserUid' => $user['_uid'], 'favourite' => 1]) ?>" data-method="post" data-callback="onFavouriteCallback" title="Favorite" class="icon lw-ajax-link-action lw-favourite-action-btn" id="lwFavouriteBtn">
                            ...
                            <?php 
                            if (in_array($user['_id'],$current_user_favourite_ids)) { ?>
